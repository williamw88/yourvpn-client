// lib/src/auth/token_store.dart
//
// Secure persistent storage for the bearer token and its expiry.
// Backed by flutter_secure_storage which uses Android Keystore (Android),
// Keychain (iOS/macOS), libsecret (Linux), and DPAPI (Windows).
//
// Add to pubspec.yaml:  flutter_secure_storage: ^9.2.2
// See pubspec.additions.yaml.

import 'package:flutter_secure_storage/flutter_secure_storage.dart';

class TokenStore {
  static const _keyToken = 'yourvpn_token';
  static const _keyExpires = 'yourvpn_token_expires';

  final FlutterSecureStorage _storage;

  TokenStore({FlutterSecureStorage? storage})
      : _storage = storage ??
            const FlutterSecureStorage(
              aOptions: AndroidOptions(encryptedSharedPreferences: true),
            );

  /// Persists [token] and its [expires] timestamp.
  Future<void> save(String token, DateTime expires) async {
    await Future.wait([
      _storage.write(key: _keyToken, value: token),
      _storage.write(key: _keyExpires, value: expires.toIso8601String()),
    ]);
  }

  /// Returns the stored token, or null if absent.
  Future<String?> readToken() => _storage.read(key: _keyToken);

  /// Returns the stored expiry, or null if absent.
  Future<DateTime?> readExpiry() async {
    final raw = await _storage.read(key: _keyExpires);
    if (raw == null) return null;
    return DateTime.tryParse(raw);
  }

  /// Removes both the token and its expiry from secure storage.
  Future<void> delete() async {
    await Future.wait([
      _storage.delete(key: _keyToken),
      _storage.delete(key: _keyExpires),
    ]);
  }

  /// Returns true when a non-expired token is stored.
  Future<bool> get isLoggedIn async {
    final token = await readToken();
    if (token == null || token.isEmpty) return false;
    final expiry = await readExpiry();
    if (expiry == null) return false;
    return expiry.isAfter(DateTime.now());
  }
}
